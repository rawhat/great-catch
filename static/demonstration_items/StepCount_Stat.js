...
async function deterDataSize(data) {
    ...
    if (stepSize == 1) {
        ...
        let heartRateMsg = compareMAD(
            parsedHeartRateData[0],
            parsedHeartRateData[1]
        );
        return (
            'Step Count ' +
            stepMsg +
        ...
        // heart rate
        let parsedHeartRateData = parseData(heartRates);
        let heartRateResult = await deterHeartRateResult(
            data,
            parsedHeartRateData[1]
        );
...
async function weatherCheck(zipcode) {
    ...
    var weatherSum =
        parseInt(mainInfo.fog) +
        parseInt(mainInfo.rain) +
        parseInt(mainInfo.snow) +
        parseInt(mainInfo.hail) +
        parseInt(mainInfo.thunder) +
        parseInt(mainInfo.tornado);
    return weatherSum;
}
...
async function drugCheck(drug, weather) {
    if (drug.toUpperCase() === 'N/A') {
        ...
        var url =
            'https://api.fda.gov/drug/event.json?api_key=' +
            key +
            '&search=reaction.reactionmeddrapt.exact=' +
            'Fatigue' +
            '+AND+brand_name:' +
            drug;
        let response = await axios.get(url);
...
function heartRateCorrelationAlert(drug, std, drugName) {
    var nothing =
        'We have found no reason why your heart rate INCREASED. We suggest you visit your PCP.';
    var starter =
        'We have found your heart rates today is ABNORMAL using sample standard deviation and correlation. But we also found that: <br>';
    var drugMsg =
        '- your medicine ' +
        drugName +
        ' might be causing HYPERTENSION by using FDA drug complaint database.<br> Here is a link to <a href=http://www.webmd.com/drugs/search.aspx?stype=drug&query=' +
        drugName +
        ' target="_blank">WEBMD</a>';
    var stepMsg =
        '- your step counts decreased or stayed the same today compared to previous dates using sample standard deviation.';
    // hard threshold for FDA drug, no reason for the number
    var drugThreshold = 1000;
